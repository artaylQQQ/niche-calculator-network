...
function defaultSampleValue(input) {
...
const publishedSlugs = new Set(log.map((r) => r.slug));
...
const pending = allCalcs.filter((c) => !publishedSlugs.has(c.slug));
...
for (const calc of toPublish) {
  ...
  // mismo grupo o del total si no hay suficientes.
  const sameCluster = allCalcs.filter((c) => c.slug !== calc.slug && c.cluster === calc.cluster);
  const pool = sameCluster.length >= 6 ? sameCluster : allCalcs.filter((c) => c.slug !== calc.slug);
  schema.related = pool.slice(0, 6).map((c) => c.slug);
